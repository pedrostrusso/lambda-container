...
# Copiando codigo e dependências para o treinamento do nosso modelo
...
# Instalando Lambda RIC - Rich Interface Client
RUN apt-get update && \
apt-get install -y \
g++ \
make \
...
# Instalando Lambda RIE - Rich Interface Emulator
...
# Copia dos arquivos de execucao
COPY app.py app.py
RUN chmod +x app.py
